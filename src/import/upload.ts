import DocumentPicker, { types, isCancel, DocumentPickerResponse } from 'react-native-document-picker';
...
const { PDFRasterizer } = NativeModules as {
  PDFRasterizer?: { rasterize: (path: string, dpi: number) => Promise<string[]> };
};
...
export async function importFromFiles(): Promise<Doc | null> {
  try {
    ...
    const now = new Date();
    const doc = newDoc(`Imported ${format(now, 'yyyy-MM-dd HH:mm')}`);
    const pages: Page[] = [];
    ...
    for (const res of results) {
      ...
      const isPDF = (fileType?.includes('pdf') ?? false) || fileName.toLowerCase().endsWith('.pdf');
